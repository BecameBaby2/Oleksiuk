...
class Report:
    ...
    def read_file(self):
        ...
        with open(self.filename, encoding="utf-8") as file:
            ...
            for str in reader:
                if not point:
                    point = True
                    designation = str.index("last")
                    #Изменённая строка
                    salaryfrom = str.index("аааааааа")
                    salaryto = str.index("salary_to")
                    sal = str.index("salary_currency")
                    ...
                    if all(str):
                        ...
                        curSalary = (int(float(str[salaryto])) + int(float(str[salaryfrom]))) * money[
                            str[sal]] // 2
    ...
    def calculate_file(self):
        ...
        neededCities = [city for city in self.cities if self.cityLength[city] >= self.vacLength // 100]
        self.ansSums = {key: self.citySum[key] for key in
                              sorted(neededCities, key=lambda x: self.citySum[x], reverse=True)[:10]}
        self.part = {key: float("{:.4f}".format(self.cityLength[key] / self.vacLength)) for key in sorted(neededCities, key=lambda x: self.cityLength[x] / self.vacLength, reverse=True)[:10]}
